...
bufRawDataReg EMRegInterpreter::getBufferDataRaw(const uint16_t* data_readed, const uint16_t size) {
    bufRawDataReg res;
    res.buffer = _MB_ReqSeccionStruct[0].RawDataBuffer; 
    res.size = 0;
    ...
    for (int i = 0; i < _MB_ReqSeccionStruct[0].registrySize; i++) {
        coded_format fmt = _MB_ReqSeccionStruct[0].registryBuffer[i].format;
        int regs_del_dato = getFormatSize(fmt);

        if (offsetOriginal + regs_del_dato > size) break;

        _MB_ReqSeccionStruct[0].RawDataBuffer[i].format = fmt;
        
        // Populate the 16-bit internal buffer for each data point
        for (int j = 0; j < regs_del_dato; j++) {
            _MB_ReqSeccionStruct[0].RawDataBuffer[i].data[j] = data_readed[offsetOriginal + j];
        }

        // Clear remaining space in buffer
        for (int j = regs_del_dato; j < MAX_DATA_SIZE; j++) {
            _MB_ReqSeccionStruct[0].RawDataBuffer[i].data[j] = 0;
        }
...
nameColValues EMRegInterpreter::getLastNameValues() {
    ...
    for (int i = 0; i < _MB_ReqSeccionStruct[0].registrySize; i++) {
        if (_MB_ReqSeccionStruct[0].registryBuffer[i].logEnabled) {
            if (count < MAX_MODBUS_REGS) {
                res.buffer[count] = _MB_ReqSeccionStruct[0].registryBuffer[i].name;
                count++;
            }
        }
    }
...
EM_request EMRegInterpreter::getLastEMRequest(){
    return _MB_ReqSeccionStruct[0].current_request; 
}
...
netDataString EMRegInterpreter::getBufNetDataString() {
    ...
    for (int i = 0; i < _MB_ReqSeccionStruct[0].registrySize; i++) {
        if(_MB_ReqSeccionStruct[0].registryBuffer[i].logEnabled){
            getNetDataString(_MB_ReqSeccionStruct[0].netDataStringBuffer[i], _MB_ReqSeccionStruct[0].RawDataBuffer[i]);

            if (count < MAX_MODBUS_REGS) {
                res.buffer[count] = _MB_ReqSeccionStruct[0].netDataStringBuffer[i];
                count++;
            }
        }    
    }
